...
![系统首页与项目总览](screenshots/readme/用户首页.png)

截图建议：展示候选人首页或系统首页，画面中最好能看到心理画像、岗位推荐或评估入口，用于证明系统具备完整入口和真实页面。
...
![岗位建模与岗位管理](screenshots/readme/岗位管理页.png)

截图建议：展示人力资源端岗位管理、岗位编辑或岗位详情页面，重点截出岗位名称、岗位要求、人格特质要求或岗位列表。
...
![评估会话与面试记录](screenshots/readme/面试模块.png)

截图建议：展示“我的面试”、面试列表、邀请记录或某个评估任务状态，重点截出岗位、评估状态、继续面试或查看报告入口。
...
![多智能体面试过程](screenshots/readme/面试模块.png)

截图建议：展示智能面试间或多轮问答页面，画面中最好能看到系统提问、候选人回答输入区、面试进度或下一步操作。

### 3.4 人格评分：区分基础人格与场景人格

系统以大五人格模型作为心理特质评估基础。基础人格用于描述候选人相对稳定的人格倾向，场景人格用于描述候选人在特定岗位情境中的行为表现。

二者的区分可以让报告既有总体人格画像，也能体现候选人在具体岗位要求下的适配情况，从而服务于后续的人岗匹配分析。

![人格评分与心理画像](screenshots/readme/心理解读模块.png)

截图建议：展示心理画像、人格雷达图、大五人格维度得分或人格解释区域，重点体现评分结果不是纯文本，而是有可视化表达。

### 3.5 匹配报告：强调可解释性链路

系统最终生成的评估报告不只展示一个总分，还包括人格维度得分、岗位匹配拆解、证据说明和改进建议。报告的目标是让人力资源人员和候选人理解“为什么匹配”或“为什么不完全匹配”。

![人岗匹配评估报告](screenshots/readme/详细报告页.png)

截图建议：展示评估报告详情页，画面中最好能看到总匹配度、人格雷达图、维度拆解、优势分析、风险提示或建议内容。
...
以下截图用于从功能视角补充系统完整性。若前面的设计截图已经覆盖，可复用同一张图；如果希望 README 更有展示效果，建议每张图都截取不同页面。

### 4.1 候选人端

候选人端主要完成岗位浏览、岗位详情查看、面试参与、人格画像查看和评估报告查看。候选人可以从岗位页面进入评估流程，也可以在“我的面试”中继续已创建的评估会话。

![候选人端核心页面](screenshots/readme/岗位详情页.png)

截图建议：展示岗位浏览、岗位详情、候选人首页或我的面试页面，重点体现候选人能从岗位进入评估流程。
...
![人力资源端首页](screenshots/readme/HR首页.png)

截图说明：人力资源端首页展示岗位管理、候选人概览和评估入口，体现系统支持招聘管理侧的业务闭环。

![候选人管理页面](screenshots/readme/候选人管理页.png)

截图说明：候选人管理页面用于展示不同岗位下候选人的评估状态、匹配结果和报告入口，便于人力资源人员进行筛选与跟进。

![招聘评估数据分析](screenshots/readme/数据分析页.png)
